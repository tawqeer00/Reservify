...
const getPastBooking = asyncHandler(async (req, res) => {
  ...
  const pastBookings = await Booking.find({
    ...
    .populate({
      ...
      path: "venue",
      select: { name: 1, }, // Populate the name and location fields of the venue
    })
    .sort({ date: -1 })
...
const getFutureBooking = asyncHandler(async (req, res) => {
  ...
  const futureBookings = await Booking.find({
    ...
    .populate({
      ...
      path: "venue",
      select: { name: 1, }, // Populate the name fields of the venue
    })
 .sort({ date: 1 })
...
const fetchLogs = asyncHandler(async (req, res) => {
  ...
  if (req.params.type == "all") {
    ...
    }).populate('venue', 'name').sort({ createdAt: "descending" })
